...
const Body = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const restaurants =
        jsonData?.data?.data?.cards?.[1]?.card?.card?.gridElements
    ...
    <div className="body">
      <div>
        <input
          ...
          className=" m-2.5 border border-grey"
          value={searchText}
          onChange={(e) => setSearchText(e.target.value)}
        />
        <button className="m-2.5" onClick={searchRestraunts}>
          Search{" "}
        </button>
        <button className="m-2.5 cursor-pointer" onClick={filterTopRated}>
          Top Rated Restraunts
        </button>
        <button className="m-2.5 cursor-pointer" onClick={removeFilters}>
          Remove Filters
        </button>
        <input
          type="text"
      ...
      <div className="flex m-2.5 flex-wrap gap-5">
        {resFilList.map((res) =>
          res.info.veg ? (
            <PromotedRestroCardComp key={res.info.id} res={res.info} />
          ) : (
            <RestrauntCard key={res.info.id} res={res.info} />
          ),
        )}
      </div>
    </div>
